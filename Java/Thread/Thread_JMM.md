...
同步是指程序用于控制不同线程之间操作发生相对顺序的机制（锁机制）。在共享内存并发模型里，同步是显式进行的。程序员必须显式指定某个方法或某段代码需要在线程之间互斥执行。

在消息传递的并发模型里，由于消息的发送必须在消息的接收之前，因此同步是隐式进行的，例如golang。
...
现代的处理器使用写缓冲区临时保存向内存写入的数据。写缓冲区可以保证指令流水线持续运行，它可以避免由于处理器停顿下来等待向内存写入数据而产生的延迟。同时，通过以批处理的方式刷新写缓冲区，以及合并写缓冲区中对同一内存地址的多次写，减少对内存总线的占用。虽然写缓冲区有这么多好处，但每个处理器上的写缓冲区，仅仅对它所在的处理器可见。

![](./img/cache_coherence_1.png)
![](./img/cache_coherence_2.png)
...
![](./img/JVM_reorderings_2.png)
 
上面3种情况，只要重排序两个操作的执行顺序，程序的执行结果就会被改变。

#### 控制依赖性

![](./img/JVM_reorderings_3.png)
...
![](./img/memory_barrier_1.png)

StoreLoad Barriers是一个“全能型”的屏障，它同时具有其他3个屏障的效果，所以开销也是最大的。现代的多处理器大多支持该屏障（其他类型的屏障不一定被所有处理器支持）。

### 7.2 临界区

![](./img/critical_sections_1.png)
...
内存语义：可以把对volatile变量的单个读/写，看成是使用同一个锁对这些单个读/写操作做了同步

![](./img/volatile_mem_1.png)
![](./img/volatile_mem_2.png)
...
![](./img/volatile_write_1.png)

**volatile读的内存语义如下：当读一个volatile变量时，JMM会把该线程对应的本地内存置为无效。线程接下来将从主内存中读取共享变量。**

![](./img/volatile_read_1.png)

**volatile重排序规则表:**

![](./img/volatile_reordering_1.png)

#### JMM对volatile的内存屏障插入策略

**在每个volatile写操作的前面插入一个StoreStore屏障（不能让上面的普通写和volatile写重排序）。在每个volatile写操作的后面插入一个StoreLoad屏障（保证写的都会被后面读到）。**

![](./img/volatile_write_2.png)

**在每个volatile读操作的后面插入一个LoadLoad屏障（防止读取到脏数据）。在每个volatile读操作的后面插入一个LoadStore屏障（保证后买的值使用的是最新的数据）。**

![](./img/volatile_read_2.png)
...
<figure class="half">
  <img src="./img/lock_memory_sema_1.png">
  <img src="./img/lock_memory_sema_2.png">
</figure>
...
![](./img/sync_mechanism_1.png)

### 7.7 了解各种锁

锁一共有4种状态（自旋锁不属于所得某种状态，属于拿锁的机制），级别从低到高依次是：无锁状态、偏向锁状态、轻量级锁状态和重量级锁状态。偏向所锁，轻量级锁都是乐观锁，重量级锁是悲观锁。
...
来源：[偏向锁，轻量级锁与重量级锁的区别与膨胀 - choukekai的博客 - CSDN博客](https://blog.csdn.net/choukekai/article/details/63688332)

![](./img/other_lock_1.png)
